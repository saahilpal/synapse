...
### What Synapse Is
Synapse is a **local, git-native project intelligence engine**. It maintains a living, structured understanding of a codebase — what exists, what it means, what happened, what failed — and injects this into AI agents via MCP before every session.

The agent never needs to ask "what does this project do." Synapse already told it.
...
2. NOTHING HAPPENS SILENTLY
   ...
   User always knows what Synapse is doing.
...
6. MIRROR GIT EXACTLY
   Every git action has a Synapse reaction.
   Index state always matches git state.
```

### What Synapse Is NOT
- Not a RAG system (agent does not query, Synapse injects)
...
```
[Synapse] Watching /home/user/myproject on branch main
[Synapse] Daemon started — autostart mode
[Synapse] MCP server running on port 7822
[Synapse] Web UI running on port 7823
```
...
**One database only:** `synapse.db`

**File layout:**
```
.synapse/
  config.json      ← user preferences
  synapse.db       ← L1 code graph + L3 memory
...
synapse init
    │
    ├── [1] Environment check (synapse doctor inline)
    │       git repo exists?
    ...
    │
    ├── [4] Daemon mode
    │       Autostart (system service) / Manual (synapse start)
    │
...
git commit
    ...
    ├── Notify CLI: "[Synapse] 4 files reindexed · wiki updated"
    └── Update active_state in synapse.db
```
...
git revert (detected by daemon)
    ...
    │     Run `synapse lessons review` to approve.
    │
...
Agent monitors own token usage
    ...
    ├── CLI notification: "[Synapse] Agent checkpointed at 60% — session saved"
    │
...
def load_api_key(provider: str) -> str:
    ...
    key = os.environ.get(f"SYNAPSE_{provider.upper()}_API_KEY")
    if key:
        return key
    ...
    try:
        import keyring
        key = keyring.get_password("synapse", provider)
        if key:
            return key
    except Exception:
        logger.warning("keyring unavailable — trying file fallback")

    # 3. Encrypted credentials file (~/.synapse/credentials, chmod 600)
    cred_path = Path.home() / ".synapse" / "credentials"
    if cred_path.exists():
    ...
    raise SynapseConfigError(
        ...
        f"  Set SYNAPSE_{provider.upper()}_API_KEY\n"
        f"  or run: synapse setup\n"
    )
```
...
Output:
  [Synapse] Starting daemon...
  [Synapse] Watching /home/user/myproject
  [Synapse] MCP server → port 7822
  [Synapse] Web UI    → localhost:7823
  [Synapse] Ready. Press Ctrl+C to stop.
```
...
```
synapse lessons           # list all lessons
synapse lessons review    # interactive approval flow for pending lessons
synapse lessons show ID   # show a specific lesson in detail
synapse lessons delete ID # delete an approved lesson
```

**`synapse lessons review` interactive flow:**
```
...
```
synapse checkpoint         # create checkpoint of current agent state
synapse checkpoint list    # list all checkpoints
synapse checkpoint restore # restore most recent checkpoint
synapse checkpoint restore --id ID
```
...
```
synapse wiki              # open localhost:7823/wiki
synapse wiki update       # force regenerate entire wiki
synapse wiki update --module src/auth   # regenerate one module only
synapse wiki show overview              # print overview.md to terminal
```
...
#### `synapse recover`
Rebuild index from git history if synapse.db is corrupted.
```
synapse recover

Output:
  ✗ synapse.db appears corrupted
...
```
synapse mcp status     # show connection health
synapse mcp config     # output config block for agent setup
synapse mcp verify     # verify agent can connect
synapse mcp restart    # restart MCP server
```

**`synapse mcp config` output:**
```
...
Run `synapse mcp verify` after adding to confirm connection.
...
Daemon mode:
  ...
  [2] Manual — start with `synapse start` when needed
...
Every git action maps to a Synapse reaction. All automatic. All visible.

```
Git Action               Synapse Reaction
──────────────────────────────────────────────────────────────────
git commit               Reindex changed files
                         Update wiki for changed modules (if score > threshold)
                         Log to activity table
                         CLI: "[Synapse] 4 files reindexed · 2min"
...
git stash               Snapshot current context to checkpoints table
                         Mark as stash type
                         CLI: "[Synapse] Context stashed"

git stash pop           Restore stash checkpoint
                         CLI: "[Synapse] Context restored from stash"

git tag                 Record tag in active_state metadata
                         CLI: "[Synapse] Tag v1.0.0 recorded"
```
...
User notified in CLI
    ↓
          User reviews in `synapse lessons review`
         /              |               \
    APPROVE           EDIT            REJECT
        ↓               ↓               ↓
...
def get_wiki_page(module: str) -> str:
    ...
    """
    path = f".synapse/wiki/modules/{module}.md"
...
def checkpoint(
    ...
    notify_cli(f"[Synapse] Checkpoint saved — session state preserved")
...
def signal_low_context(token_count: int, capacity: int) -> dict:
    ...
    Synapse prompts agent to checkpoint immediately.
    """
...
CASE: Repository has no commits yet (empty repo)
  synapse init must fail clearly:
...
CASE: API key expires mid-session
  ...
  CLI: "✗ API key invalid. Run: synapse setup"
...
```
CASE: Two Synapse processes started simultaneously
  Use a lock file: .synapse/daemon.lock
  Second process: "✗ Synapse already running (PID 12345)"
...
CASE: Machine goes to sleep mid-index
  ...
  Never leave synapse.db in partial state (SQLite WAL handles this)

CASE: Disk full during indexing
  Catch disk write errors
  CLI: "✗ Disk full — indexing stopped. Free space and run: synapse index"
  Do not corrupt existing index

CASE: .synapse/synapse.db corrupted
  Detect on startup: run PRAGMA integrity_check
  If fails: "✗ Index corrupted. Run: synapse recover"
...
CASE: Checkout with uncommitted changes (git blocks it)
  Git will block, Synapse does nothing
...
CASE: Branch deleted remotely, still tracked locally
  ...
  Synapse mirrors local git state only
...
CASE: Agent crashes without checkpointing
  ...
  Synapse injects: "⚠ No checkpoint found — agent may have exited unexpectedly"
...
CASE: Multiple checkpoints on same branch
  ...
  `synapse checkpoint list` shows all, user can pick specific one
```
...
```
REMOVE: llm_provider = "mock" from SynapseSettings
  Replace with: explicit Mode A (no provider) with clear messaging
  Mock belongs in test fixtures ONLY

REMOVE: synapse trace command
  Replaced by: synapse doctor --context
  Philosophy: agent should not ask specific questions

REMOVE: synapse index --full as separate flag
  Replaced by: synapse recover (for corruption)
  and: synapse rollback (for intentional reset)

REMOVE: synapse stash command
  Git stash is automatically mirrored by daemon
  No manual CLI command needed

REMOVE: synapse diff command
  Covered by synapse status output

REMOVE: synapse memory log as separate command
  Covered by synapse memory --full
...
*End of Synapse Build Specification v1.0*
