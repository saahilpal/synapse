...
### `synap cost`
Shows aggregated LLM calls, token counts, and estimated USD cost. This command is an alias for `synap usage show` and prints the same aggregated table, including an estimated cost total.
